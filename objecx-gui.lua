...
function gui.add.window(name, properties)
	...
	window.Name = name
	window.Size = UDim2.new(0, (properties.size and properties.size.X.Offset) or (400 * scale), 0, totalHeight)
	window.Position = properties.position or UDim2.new(0.5, -((properties.size and properties.size.X.Offset or 400 * scale) / 2), 0.5, -(totalHeight / 2))
...
-- Create element (same as before, keeping sliders thicker)
function createElement(windowName, elementData, index, targetContainer)
	...
	if elementData.type == "console" then
		...
		consoles[windowName] = {
			...
			maxLines = math.floor((elementData.size and elementData.size.Y.Offset or 150 * scale) / (13 * scale)),
			lineHeight = 13 * scale,
			scale = scale
		}
	...
	elseif elementData.type == "slider" then
		...
		sliderBg.Size = UDim2.new(1, 0, 0, 6 * scale)  -- Increased from 4 to 6
		...
		sliderHandle.Size = UDim2.new(0, 12 * scale, 0, 12 * scale)  -- Increased from 10 to 12
...
-- Console functions (same as before)
function gui.console.addLine(windowName, text, textColor)
	...
	local textBounds = textService:GetTextSize(text, 10 * scale, Enum.Font.RobotoMono, Vector2.new(lineLabel.AbsoluteSize.X, math.huge))
...
-- Create notification (same as before with updated fonts)
