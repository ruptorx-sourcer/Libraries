...
-- Calculate window height based on elements (FIXED)
local function calculateWindowHeight(elements, scale)
	...
	-- Only add minimal padding (reduced from 8 to 5)
	local extraSpace = 5 * scale
	
	-- Only add extra for dropdown if it's the last element
	if lastElement and lastElement.type == "dropdown" then
		extraSpace = extraSpace + (20 * scale)
	end
	
	-- Only add extra for label if it's the last element
...
function createElement(windowName, elementData, index, targetContainer)
	...
	if elementData.type == "console" then
		...
		if elementData.isScrollable then
			...
			scrollFrame.ScrollBarThickness = 3 * scale
	...
	elseif elementData.type == "slider" then
		...
		sliderBg.Position = UDim2.new(0, 0, 0, 14 * scale)
		...
		sliderHandle.Size = UDim2.new(0, 10 * scale, 0, 10 * scale)
		sliderHandle.Position = UDim2.new(1, -(5 * scale), 0.5, -(5 * scale))
